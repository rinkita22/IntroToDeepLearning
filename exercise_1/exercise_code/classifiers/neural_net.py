...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #############################################################################
        W1x = np.dot(X,W1) + b1
        h1 = np.maximum(0, W1x) # pass through ReLU activation function
        h2 = np.dot(h1,W2) + b2
        scores = h2
        #############################################################################
        #                              END OF YOUR CODE                             #
        ...
        # classifier loss. So that your results mat`1Qch ours, multiply the            #
        # regularization loss by 0.5                                                #
        #############################################################################
        # compute the class probabilities
        h2exp = np.exp(h2)
        probs = h2exp / np.sum(h2exp, axis=1, keepdims=True) # [N x K]
    
        # average cross-entropy loss and regularization
        log_rightprobs = -np.log(probs[range(N), y])
        data_loss = np.sum(log_rightprobs) / N
    ...
    def predict(self, X):
        ...
        """
        y_pred = None
        W1x = X.dot(self.params['W1']) + self.params['b1']
        h1 = np.maximum(0, W1x) # pass through ReLU activation function
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    best_net = None  
    learning_rates = [0.9]
    regularization_strengths= [1.5e-7] 

    for lr in learning_rates:
        for reg in regularization_strengths:
            net = TwoLayerNet(X_train.shape[1],500,10)
            net.train(X_train, y_train, X_val, y_val,
              lr, learning_rate_decay=0.95,
              reg=1.5e-7, num_iters=5000,
              batch_size=256, verbose=False)
            y_val_pred = net.predict(X_val)            
            y_train_pred = net.predict(X_train)
            train_accuracy = np.mean(y_train == y_train_pred)
            val_accuracy = np.mean(y_val == y_val_pred)
            print("val_accu {} train acc {} lr {} reg {}".format(val_accuracy,train_accuracy,lr,reg))
