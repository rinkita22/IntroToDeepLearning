...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        conv_out_dim1 = int((height + 2*self.padding - kernel_size)/stride_conv) + 1
        conv_out_dim2 = int((width + 2*self.padding - kernel_size)/stride_conv) + 1
        pool_out_dim1 = int((conv_out_dim1 - self.pool_filter)/self.stride_pool) + 1
        pool_out_dim2 = int((conv_out_dim2 - self.pool_filter)/self.stride_pool) + 1
        self.conv = nn.Conv2d(channels, num_filters, kernel_size, stride=stride_conv, padding=self.padding)
        self.conv.weight.data.mul_(weight_scale)
        self.fc1 = nn.Linear(num_filters*pool_out_dim1*pool_out_dim2, hidden_dim)
        self.out_dim = num_filters*pool_out_dim1*pool_out_dim2
    ...
    def forward(self, x):
        ...
        # layers.                                                              #
        ########################################################################
        x = F.max_pool2d(F.relu(self.conv(x)),kernel_size=self.pool_filter,stride=self.stride_pool)        
        
        x = x.view(x.size(0), -1) 
        x = x.view(-1,int(self.out_dim))
